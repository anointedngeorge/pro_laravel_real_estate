...
export default function Client({ ...pageData }) {
    ...
    const onDestroy = (object) => {
        router.delete(route('client.destroy', object.id));
    }
        ...
        <AuthenticatedLayout
            ...
            header={
                <div className='flex justify-between'>
                    ...
                    <Link className='px-2 py-2 rounded items-center hover:bg-green-600 bg-green-500 text-white' href={route('client.create')}>Add Client</Link>
                </div>
            }
        >
            <Head title="Client" />
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <div className="p-6 text-gray-900">
                            <div className="pagination-footer">
                                <Pagination links={pageData.client.meta.links} />
                            </div>
                            <div className='overflow-auto'>
                                ...
                                >
                                    ...
                                    <tbody className='odd:bg-white even:bg-gray-50'>
                                        {pageData.client.data.map((client, index) => (
                                            <tr key={client.id}>
                                                <TableRow>{index + 1}</TableRow>
                                                <TableRow>{client.fullname}</TableRow>
                                                <TableRow>{client.phone}</TableRow>
                                                <TableRow>
                                                    <ul className='text-nowrap flex gap-x-2 px-2 py-3'>
                                                        <li>
                                                            <Link className='text-blue-500' href={route('client.edit', client.id)}>Edit</Link>
                                                        </li>
                                                        <li>
                                                            <Link className='text-amber-500' href={route('client.show', client.id)}>View</Link>
                                                        </li>
                                                        <li>
                                                            <button className='text-red-500' onClick={e => onDestroy(client)}>Delete</button>
                                                        </li>
                                                    </ul>
                                                </TableRow>
                                            </tr>
